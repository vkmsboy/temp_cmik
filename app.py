...
# Safely convert ADMIN_USER_ID to integer
try:
...
def manga_detail(manga_id):
    ...
    if not manga:
        session.close()
        abort(404)
    # Try to sort chapters numerically, fallback to string sort
    try:
...
def get_telegram_image(file_id):
    """
    Proxy route to get a temporary public URL for a Telegram image.
    This prevents exposing your bot token in the frontend.
    """
...
(
    CHOOSE_ACTION, ASK_MANGA_TITLE, ASK_MANGA_DESC, ASK_MANGA_COVER,
    CHOOSE_MANGA_FOR_CHAPTER, ASK_CHAPTER_NUMBER, UPLOAD_PAGES
) = range(7)

async def admin_only(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    if not ADMIN_USER_ID:
        if update.message:
            await update.message.reply_text("Bot is not configured with an admin ID. Uploads are disabled.")
        logger.warning("Upload command used, but ADMIN_USER_ID is not configured.")
        return False
...
async def start_upload(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    keyboard = [
        [InlineKeyboardButton("Add New Manga", callback_data="add_manga")],
...
async def save_manga_cover(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        new_manga = Manga(
            title=context.user_data["manga_title"],
            description=context.user_data["manga_desc"],
            cover_file_id=context.user_data["manga_cover_id"]
        )
...
async def choose_manga_for_chapter(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    await query.edit_message_text(text="Which manga are you adding a chapter to?", reply_markup=reply_markup)
...
async def start_page_upload(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(
        "OK. Now, send me all the page images for this chapter, one by one.\n"
...
async def save_chapter(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        new_chapter = Chapter(
            ...
            chapter_number=context.user_data["chapter_number"]
        )
        session.add(new_chapter)
        session.flush()

        for i, file_id in enumerate(context.user_data["pages"]):
            new_page = Page(chapter_id=new_chapter.id, page_number=i + 1, file_id=file_id)
            session.add(new_page)
        
        session.commit()
        manga = session.query(Manga).get(context.user_data["manga_id"])
        await update.message.reply_text(
            f"✅ Success! Chapter {new_chapter.chapter_number} of '{manga.title}' has been added with {len(context.user_data['pages'])} pages."
        )
    except Exception as e:
        session.rollback()
...
def run_bot():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            CHOOSE_ACTION: [
                CallbackQueryHandler(ask_manga_title, pattern="^add_manga$"),
                CallbackQueryHandler(choose_manga_for_chapter, pattern="^add_chapter$"),
            ],
            ...
            UPLOAD_PAGES: [
                MessageHandler(filters.PHOTO, collect_page),
                CommandHandler("done", save_chapter),
            ],
        },
